...
data  <- cellWise::generateData(n, d, mu, Sigma, perout = 0.2,
                      gamma = 4, outlierType) 
x <- data$X
pairs(x[,1:5])
...
p1 = ggplot(data = summary, aes(x = gamma,y = F_norm, color = Method ))+geom_line()+geom_point()+facet_wrap(~e) +
...
p2 = ggplot(data = summary, aes(x = gamma,y = KL_div, color = Method ))+geom_line()+geom_point()+facet_wrap(~e) +
  scale_x_continuous(breaks=seq(0,12,2)) +
  #ylim(0,20) +
