...
# Simulation
In our simulations, we set $n = 100$, $p = 15$, $\mathbf{x}_i$ is sampled from $N(\pmb 0,\pmb\Sigma_{xx})$. The correlation structure among variables is given by $\Sigma _{ij} = \rho^{|i-j|}$ and we set $\rho = 0.5$. 
...
summary = summary %>% mutate(Method = factor(mtd, levels = c("MCD",  "OGK", "Pair", "Cell_weight","Cell_weight_group")))
